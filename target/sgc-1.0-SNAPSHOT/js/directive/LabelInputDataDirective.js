...
sgcApp.directive('labelInputData', ['$translate', '$filter', function($translate, $filter) {
	...
	return {
		...
		replace : true,
		restrict : 'E',
		templateUrl : '/sgc/js/directive/html/labelInputDate.html',
				...
				if(data && data !== ""){
					...
					if(data.length === 10){
						...
						if(!fn) {
							...
							angular.element('#avaliacaoApp-controller').scope().showMessageErrorCampoInvalido($translate.instant($scope.label));
						} else {
							$scope.dataInvalida = false;
						}
					} else {
